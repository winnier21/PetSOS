...
function NavigationPage() {
  const mapUrl = "https://storage.googleapis.com/maps-solutions-phrazvpkye/commutes/p9jf/commutes.html"
  const [setMapUrl] = useState('');

  useEffect(() => {
    // Ensure the fetch URL is correct and potentially prefixed with your API route if necessary
    fetch('/api/navigation/getPlaces')  // Adjust the URL based on your server's endpoint configuration
      .then(response => {
        if (!response.ok) {
